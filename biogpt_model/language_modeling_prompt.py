...
from biogpt_model.language_model_prompt_dataset import LanguageModelPromptDataset
...
class LanguageModelingPromptConfig(LanguageModelingConfig):
	...
	max_source_positions: Optional[int] = field(
		default=384,
		metadata={"help": "max number of tokens in the source sequence, exclude eos."},
	)
	manual_prompt: Optional[str] = field(
		default=None,
		metadata={
			"help": "manual prompt to use",
		},
	)
	...
	learned_prompt_pattern: Optional[str] = field(
		default="learned",
		metadata={
			"help": "pattern of virtual tokens, default is learned",
		},
	)
	...
	sep_token: Optional[str] = field(
		default="<seqsep>",
		metadata={"help": "token to seperate prompt source and target."},
	)
...
class LanguageModelingPromptTask(LanguageModelingTask):
	...
	def __init__(
		self, args, dictionary, output_dictionary=None, prompt=None, targets=None
	):
	...
	def setup_prompt(cls, args, dictionary):
		...
		assert not (
			args.manual_prompt and args.learned_prompt
		), "manual prompt and learned prompt can not be set "
	...
	def load_dataset(
		...
		if split_exists(
			...
			prefix = os.path.join(
				data_path,
				"{}.{}-{}.".format(split, self.args.source_lang, self.args.target_lang),
			)
		else:
	...
	def build_dataset_for_inference(
		self, src_tokens, src_lengths, tgt_tokens=None, tgt_lengths=None
	):
		"""
	...
	def inference_step(
		...
		with torch.no_grad():
			...
			if allowed_text is not None:
				allowed_text = self.target_dictionary.encode_line(
					allowed_text, add_if_not_exist=False
				).to(sample["net_input"]["src_tokens"])

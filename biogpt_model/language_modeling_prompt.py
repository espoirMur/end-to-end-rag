...
class LanguageModelingPromptTask(LanguageModelingTask):
    ...
    def __init__(self, args, dictionary, output_dictionary=None, prompt=None, targets=None):
        ...
        self.prompt = prompt
        self.prompt_length = self.prompt.size(
            0) if self.prompt is not None else 0
    ...
    def setup_prompt(cls, args, dictionary):
        ...
        assert not (
            args.manual_prompt and args.learned_prompt), "manual prompt and learned prompt can not be set "
        if args.manual_prompt and len(args.manual_prompt) != 0:
            prompt = dictionary.encode_line(
                args.manual_prompt, append_eos=False).long()
    ...
    def setup_dictionary(cls, args, **kwargs):
        ...
        if args.data:
            ...
            assert len(paths) > 0
            dictionary = Dictionary.load(os.path.join(
    ...
    def load_dataset(
        ...
        if split_exists(split, self.args.source_lang, self.args.target_lang, self.args.source_lang, data_path):
            prefix = os.path.join(
                data_path, "{}.{}-{}.".format(split, self.args.source_lang, self.args.target_lang))
        else:
    ...
    def build_dataset_for_inference(self, src_tokens, src_lengths, tgt_tokens=None, tgt_lengths=None):
        ...
        if tgt_tokens is None:
            tgt_tokens = [torch.LongTensor(
                [self.dictionary.eos()]) for _ in range(bs)]
        ...
        with torch.no_grad():
            ...
            if allowed_text is not None:
                allowed_text = self.target_dictionary.encode_line(
                    allowed_text, add_if_not_exist=False).to(sample['net_input']['src_tokens'])
    ...
    def build_generator(
        ...
        if prefix_allowed_tokens_fn is None:
            prefix_allowed_tokens_fn = getattr(
                args, "prefix_allowed_tokens_fn", None)
        if (
            sum(
                int(cond)
                for cond in [

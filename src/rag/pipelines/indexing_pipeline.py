...
from hera.workflows import Container, Workflow, Steps, Step, WorkflowsService, Resources, SecretEnv
...
postgres_host = SecretEnv(name='POSTGRES_HOST', secret_key="POSTGRES_HOST",
                          secret_name="postgres-database-secrets")
postgres_password = SecretEnv(
    name='POSTGRES_PASSWORD', secret_key="POSTGRES_PASSWORD", secret_name="postgres-database-secrets")
postgres_user = SecretEnv(
    name='POSTGRES_USER', secret_key="POSTGRES_USER", secret_name="postgres-database-secrets")
postgres_port = SecretEnv(
    name='POSTGRES_PORT', secret_key="POSTGRES_PORT", secret_name="postgres-database-secrets")
postgres_database = SecretEnv(
    name='POSTGRES_DB', secret_key="POSTGRES_DB", secret_name="postgres-database-secrets")

secrets = [postgres_host, postgres_password,
           postgres_user, postgres_port, postgres_database]

...
def convert_biogpt_checkpoint_to_pytorch(
	...
	for layer_name in layer_names:
		if layer_name.endswith("output_projection.weight"):
			...
			model_state_dict[
				layer_name.replace("decoder", "biogpt")
			] = model_state_dict.pop(layer_name)

	# For the prompt model, the vocab size is large than the original model.
	if (
		model_state_dict["biogpt.embed_tokens.weight"].shape[0] > src_vocab_size
		and model_state_dict["output_projection.weight"].shape[0] > src_vocab_size
	):
		shape_difference = (
			model_state_dict["biogpt.embed_tokens.weight"].shape[0] - src_vocab_size
		)
